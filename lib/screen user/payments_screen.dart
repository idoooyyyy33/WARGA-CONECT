...
class _PaymentsScreenState extends State<PaymentsScreen> {
    ...
    if (mounted) {
      setState(() {
        ...
        if (result['success']) {
          ...
          _showSnackBar(result['message'] ?? 'Gagal memuat data pembayaran', isError: true);
        }
      });
    }
  }
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: const DecorationImage(
                      ...
                      image: NetworkImage('https://i.ibb.co/L0xJ1pX/qr-code-placeholder.png'),
                      fit: BoxFit.cover,
                    ),
                  ),
                ),
  ...
  Future<void> _uploadProof(String paymentId) async {
    try {
      ...
      final result = await _apiService.uploadPaymentProof(paymentId, imageBytes, fileName);

      if (mounted) {
        Navigator.pop(context); // Tutup loading
        
        if (result['success']) {
          _showSnackBar('Bukti bayar berhasil di-upload. Menunggu verifikasi admin.', isError: false);
          _loadPayments(); // Refresh list
        } else {
          _showSnackBar(result['message'] ?? 'Gagal meng-upload bukti', isError: true);
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // --- PERUBAHAN --- Menghapus FAB karena tombol bayar ada di kartu
            ...
            Row(
              children: [
                ...
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                  decoration: BoxDecoration(
                    color: _getStatusColor(status).withOpacity(0.2), // Pakai status
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text(
                    status, // Pakai status
            ...
            if (!isPaid) ...[
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: isPending ? null : () { // Nonaktifkan jika sedang "Menunggu Verifikasi"
                    _showPaymentDialog(payment);
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: isPending ? Colors.grey : const Color(0xFF10B981),
